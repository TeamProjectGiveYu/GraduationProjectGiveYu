...
export default class LogIn extends React.Component {
  ...
  login() {
    ...
    axios
      .post('http://192.168.1.7/GP_backend/Login.php', {
  ...
  //         return false;
  //     } else {
  //         this.setState({ pass_err: '', user_pass: value });
  //         return true;
  //     }
  // };
  ...
  Email_Pass_Validation() {
    ...
    // Pass Conditions
    if (this.state.user_pass.trim() == '') {
      this.setState({pass_err: 'الرجاء إدخال كلمة المرور'});
      errors++;
    } else if (this.state.user_pass.trim().length < 6) {
      this.setState({pass_err: 'كلمة المرور يجب ان تكون 6 عناصر او أكثر'});
      errors++;
    } else {
      this.setState({pass_err: ''});
    }

    // Correct Inputs Condition
    // let this.state.email_err= email_err;
    // if (this.state.email_err=='' && this.state.pass_err=='') {
